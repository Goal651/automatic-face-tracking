...
class ServoConfig:
    ...
    publish_interval: float = 0.2  # Responsive 10Hz updates for smooth tracking
...
class ServoController:
    ...
    def _publish_angle(
        ...
        """Publish servo angle command as JSON"""
        if not self.enabled:
            return

        now = time.time()
        # Only publish if enough time has passed (throttling)
        if now - self.last_mqtt_publish < self.config.publish_interval:
            return

        try:
            if abs(angle - self._last_published_angle) >= 1:
                # Construct JSON payload for Arduino
                payload = json.dumps(
                    {
                        "status": status,
                        "angle": angle,
                        ...
                        "frame": 0,  # Could be passed if needed
                        "target": target,
                    }
                )
                self.mqtt_client.publish(self.config.topic_movement, payload)

                # Update tracking state immediately for smooth control
                self._last_published_angle = angle
                self._current_servo_angle = float(angle)
    ...
    def center_servo(self):
        ...
        self._publish_angle(center_angle, status="CENTERING", target="system")
        self._scanning = False

    def update_face_tracking(
        self,
        ...
        if movement_info:
            ...
            if (
                self.config.auto_stop_on_lock
                and is_locked
                and not self._lock_centering_complete
            ):
                self._publish_angle(
                    self.config.servo_center_angle,
                    status="LOCKED_CENTER",
                    target="target",
                )
            ...
            if abs(displacement[0]) <= self.config.centering_tolerance * frame_width:
                ...
                if (
                    self._centered_stable_frames
                    >= self.config.centering_stability_threshold
                ):
                    self._lock_centering_complete = True
            else:
        ...
        target_angle = max(
            self.config.servo_min_angle, min(self.config.servo_max_angle, target_angle)
        )
        ...
        self._publish_angle(
            angle_int, status="TRACKING", confidence=confidence, target="target"
        )
    ...
    def update_scan_mode(self, current_time: float) -> Optional[int]:
        ...
        self._publish_angle(angle_int, status="SCANNING", target="none")
        return angle_int

    def process_tracking_update(
        self, tracking_result: dict, frame_size: Tuple[int, int]
    ) -> Optional[int]:
        ...
        if (
            is_locked
            and tracking_result.get("target_face")
            and tracking_result.get("match_result")
        ):
            ...
            return self.update_face_tracking(
                face_center, frame_size, is_locked, confidence
            )
        else:
...
class FaceTracker:
    ...
    def __init__(self, config: TrackingConfig):
        ...
        self.working_detector = WorkingFaceDetector(
            min_size=config.min_face_size, debug=False
        )
    ...
    def recognize_face(
        self, frame: np.ndarray, face_det: FaceDetectionResult
    ) -> MatchResult:
        ...
        center = (
            (face_det.x + face_det.x + face_det.width) // 2,
    ...
    def recognize_all_faces(
        self, frame: np.ndarray, faces: List[FaceDetectionResult]
    ) -> List[Tuple[FaceDetectionResult, MatchResult]]:
        ...
        for working_face in working_faces:
            center = (
                (working_face.x1 + working_face.x2) // 2,
                (working_face.y1 + working_face.y2) // 2,
            )
            ...
            if mr is None or needs_recognition:
                try:
                    aligned, _ = align_face_5pt(
                        frame, working_face.kps, out_size=(112, 112)
                    )
                    emb = self.embedder.embed(aligned)
                    mr = self.matcher.match(emb)
                    self.identity_cache[center] = (mr, self.frame_count)
    ...
    def find_target_face(
        self, frame: np.ndarray, faces: List[FaceDetectionResult]
    ) -> Optional[Tuple[FaceDetectionResult, MatchResult]]:
        ...
        for working_face in working_faces:
            ...
            )  # Recognize target every 5 frames (more frequent)
    ...
    def detect_actions(
        self, face: FaceDetectionResult, frame_time: float
    ) -> List[ActionRecord]:
        ...
        if movement_info:
            ...
            if self.action_classifier.should_record_action(action_type, frame_time):
                description = self.action_classifier.get_action_description(
                    action_type, movement_info
                )
                actions.append(
                    ActionRecord(
    ...
    def process_frame(self, frame: np.ndarray) -> Dict[str, Any]:
        ...
        if self.frame_count % 60 == 0:
            print(
                f"⚡ Frame processing: {process_time:.1f}ms | Faces: {len(faces)} | Cache: {len(self.identity_cache)}"
            )
...
class FaceLockingApp:
    ...
    def _create_visualization(
        ...
        vis = frame.copy()
        faces = result["faces"]
        all_recognitions = result.get(
            "all_recognitions", []
        )  # Get all face recognitions
        ...
        for face_det in faces:
            ...
            for f, mr in all_recognitions:
                if (
                    f.x1 == face_det.x1
                    and f.y1 == face_det.y1
                    and f.x2 == face_det.x2
                    and f.y2 == face_det.y2
                ):
            ...
            # Draw bounding box
            cv2.rectangle(
                vis, (face_det.x1, face_det.y1), (face_det.x2, face_det.y2), color, 2
            )
            ...
            quality_text = (
                f"Q:{face_det.quality_score:.2f}"
                if hasattr(face_det, "quality_score")
                else ""
            )
            ...
            cv2.putText(
                vis,
                label,
                (face_det.x1, max(0, face_det.y1 - 10)),
        ...
        if is_locked and target_face and match_result and match_result.accepted:
            if match_result.name == self.tracking_config.target_identity:
                ...
                cv2.putText(
                    vis,
                    "🔒 LOCKED",
                    (target_face.x1, target_face.y2 + 30),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (0, 255, 0),
                    2,
                ...
                if self.show_confidence:
                    ...
                    cv2.putText(
                        vis,
                        conf_text,
                        (target_face.x1, target_face.y1 - 30),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (0, 255, 0),
                        1,
        ...
        if self.show_detailed_info:
            info_lines.extend(
                [
                    ...
                    f"Avg confidence: {np.mean(list(tracking_state.confidence_history)):.3f}"
                    if tracking_state.confidence_history
    ...
    def _draw_ui_header(self, vis: np.ndarray, result: Dict[str, Any]):
        ...
        cv2.putText(
            vis, header_text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, status_color, 2
        )
...
def main():
    ...
    if not available_users:
        ...
        choice = input(
            f"Select user (1-{len(available_users)}) or press Enter for default: "
        ).strip()
